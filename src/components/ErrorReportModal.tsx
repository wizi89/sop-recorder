...
interface ErrorReportModalProps {
  ...
  onGrant: (comment: string | null, alwaysSend: boolean) => void;
  onDecline: () => void;
  onClose: () => void;
}
...
export function ErrorReportModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-3">
      <div className="bg-surface rounded-lg w-full max-w-sm p-4 flex flex-col gap-2.5">
        <p className="text-on-surface text-sm font-semibold">
        ...
        <div className="bg-surface-container rounded p-2 flex flex-col gap-2 max-h-56 overflow-y-auto">
          <div>
        ...
        <div className="flex gap-3 mt-1">
          ...
          <button
            onClick={() => {
              setBusy(true);
              onGrant(comment.trim() === "" ? null : comment.trim(), alwaysSend);
            }}
            disabled={busy}
            className="btn-primary flex-1 py-2 text-xs"
          >
            {loggedIn ? t("report.send") : t("report.send_after_login")}
          </button>
        </div>
      </div>
    </div>
