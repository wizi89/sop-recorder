...
- [x] 3.3 Surface the count in `ReviewScreen` as "N of M steps could not be captured" and add the German copy to `src/i18n/de.ts`. Verified by TS tests for the notice rendering (2 failures shown, 0 and absent both silent) and by two `useRecorder` tests covering the joint: `stop_recording`'s `failed_captures` reaching the state the review screen reads. Confirmed on screen on 2026-09-06: a ten-step recording whose `screenshots/` was made read-only mid-run (`chmod 500`) failed steps 2-4, and the review screen read „3 von 10 Schritten konnten nicht aufgenommen werden" — the total counts the three lost steps back in, so it names what the user did rather than what survived.
