...
- The Screen Recording row explains the `+` button. macOS lists an app in that pane only once it has requested screen capture, and is documented to refuse that for an app it does not consider properly signed -- so on an ad-hoc build the row can be missing entirely, leaving the user at a pane with nothing to switch on. Adding the app by hand always works, and the row now says so.
