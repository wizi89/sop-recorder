...
- The Screen Recording row explains the `+` button. macOS lists an app in that pane only once it has requested screen capture, and on macOS 26 that request does not put it there: the call returns, the pane opens, and no row appears. Signing was the leading suspect and has been ruled out -- a properly signed build behaves identically. Adding the app by hand with `+` always works, so the row presents that as the normal path rather than a fallback.
